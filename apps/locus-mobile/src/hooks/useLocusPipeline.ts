/**
 * useLocusPipeline — app-side wiring of the locus-sdk integrity pipeline.
 *
 * NO simulated data paths anywhere: the seven physics checks, the RAIM/FDE machine,
 ...
 */
import { LocusNet, useBarometerStream, useGnssMeasurements, useImuStream, useLocationStream } from 'locus-sdk';
import type {
  LocusSDK,
  BaroSample,
  CheckId,
...
export function useLocusPipeline() {
  ...
  // Real OS-level VPN signal (LocusNet), sampled on a 2 s poll and read at
  ...
  // LocusNet poll: sample the real OS VPN signal every 2 s into a ref so
  // each evaluation window carries the tunnel state at evaluation time.
  useEffect(() => {
    const poll = () => {
      try {
        vpnActiveRef.current = LocusNet.isVpnActive();
      } catch {
  ...
  const evaluateNow = useCallback(() => {
    ...
    const { result: v, ms } = measureDeterministic(() =>
      sdk.evaluate({
        ...
        // Real OS VPN probe; omitted until the first LocusNet poll lands so
...
export default useLocusPipeline;
